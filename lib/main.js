...
module.exports = function(O){
  ...
  S['restore'] = function(options, callback){
    ...
    a.o.filters = _.map(a.o.filters, function(f){ return new RegExp(f); });

    var gb = {};
    gb.key_regex = new RegExp('^manifests\\/' + a.o.path.replace(/\//g, '\\/'));

    gb['iter'] = function(manifests, callback){
      return Async.waterfall([
        function(cb){
          return FSTK.mkdir(a.o.dest_path, Belt.cw(cb, 0));
        }
      , function(cb){ 
          return Async.eachLimit(_.filter(manifests, function(m){
            return m.Key.match(/\.checksums\.gz$/) && !_.some(a.o.filters, function(f){
              return m.Key.match(f);
            });
          }), a.o.concurrency, function(m, cb2){
            var gb2 = {};
            return Async.waterfall([
              function(cb3){
            ...
            , function(cb3){
                ...
                log.info(gb2.file.path);
  
                return Async.eachLimit(gb2.file.parts, a.o.concurrency, function(p2, cb4){
                  if (p2.checksum.length !== 32){
                    gb2.file.data = p2.checksum;
                    return cb3();
                  }
    ...
    return Async.doWhilst(function(next){
      mark = Belt.get(res, 'Contents.999.Key');

      return s3._API.listObjects({
        'MaxKeys': 1000
      ...
      , 'Marker': mark
        ...
        return gb.iter(res.Contents, next);
      });
